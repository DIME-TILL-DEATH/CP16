...
static inline void KeyExpansion(void)
{
  uint32_t i, j, k;
...
static inline void AddRoundKey(uint8_t round)
{
  uint8_t i,j;
  for(i=0;i<4;++i)
  {
...
static inline void SubBytes(void)
{
  uint8_t i, j;
  for(i = 0; i < 4; ++i)
  {
...
static inline void InvMixColumns(void)
{
  int i;
  uint8_t a,b,c,d;
  for(i=0;i<4;++i)
...
// The SubBytes Function Substitutes the values in the
// state matrix with values in an S-box.
static inline void InvSubBytes(void)
{
  uint8_t i,j;
  for(i=0;i<4;++i)
  {
...
//-------------------------------------------------------------------------------------
static inline void InvShiftRows(void)
{
  uint8_t temp;
...
static inline void Cipher(void)
{
  uint8_t round = 0;
...
//-------------------------------------------------------------------------------------
static inline void InvCipher(void)
{
  uint8_t round=0;
...
void AES128_CBC_encrypt_buffer(uint8_t* output, uint8_t* input, const uint32_t length, const uint8_t* key, const uint8_t* iv)
{
  ...
  for(size_t i = 0; i < length; i += KEYLEN)
  {
    XorWithIv(input , ivec);
    BlockCopy(output, input);
    state = (state_t*)output;
    Cipher();
    ivec = output;
