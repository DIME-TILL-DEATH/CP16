...
enum bank_pres_field_t {
	bpf_bank = 0, bpf_preset = 1
};
...
inline auto decode_preset(const uint32_t val) {
	struct bank_preset_t {
		uint8_t bank;
		uint8_t preset;
	} bp = { (val >> 4) & 0xf, val & 0xf };
	return bp;
}
...
bool EEPROM_copyFile(emb_string &err_msg, const emb_string &srcPath,
