...
static void eq_position_comm_handler(TReadLine* rl, TReadLine::const_symbol_type_ptr_t* args, const size_t count)
{
	msg_console("%s", args[0]);
	 if(count == 1)
	 {
...
static void esc_comm_handler(TReadLine* rl, TReadLine::const_symbol_type_ptr_t* args, const size_t count)
{
	preset_change();
	msg_console("%s\r\nEND\n", args[0]);
}
