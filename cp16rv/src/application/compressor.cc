...
void Compressor_init (void)
{
  ...
  for(uint8_t i = 0 ; i < 5 ; i++)comp_par(i | (a[i] << 8));
}
...
void Gate_Change (int np, int value)
{
    ...
    case 1:Pdecay = value;d_rate = 1000.0f / ((float)Pdecay * fs);break;
    }
}
void Gate_Change_Preset (void)
{
	  ...
	  hold_count = 0;
    int presets[2] = {-50, 1};
    for (int n = 0; n < 2; n++) Gate_Change (n, presets[n]);
}
...
void comp_par(uint32_t val)
{
	...
	val &= 0xff;
	switch(val & 0xff){
	case 0:Compressor_Change(0, -(sqrtf(va) * (50.0f/sqrtf(31.0f))));break;
	case 1:Compressor_Change(1, va * (20.0f/127.0f) + 2.0f);break;
	case 2:Compressor_Change(2, va * (20.0f/31.0f));break;
