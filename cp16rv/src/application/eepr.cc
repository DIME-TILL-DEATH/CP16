...
uint8_t imp_buf_uint[3938] = {0,2,0,0x80};
const uint8_t stas[] = {"STAS"};
...
constexpr char volume_label[] = FIRMWARE_NAME ; //"CP16" ;

extern "C" char* uitoa(int val, char* buf, int base) ;
...
void save_pres(void)
{
	...
	f_lseek (&file , seek);
	f_write(&file, prog_data , 128 , &f_size);
	f_sync(&file);
	f_close(&file);
	f_mount(0, "0:", 0);
}
