...
volatile uint32_t buff_size = 32768;
...
static void amp_on_command_handler ( TReadLine* rl , TReadLine::const_symbol_type_ptr_t* args , const size_t count )
{
	 if ( count == 1 )
	 {
		 i2hex( prog_data[amp_on],hex);
		 msg_console("%s\n" , hex ) ;
		 return ;
	 }
	 char* end ;
     uint32_t val = kgp_sdk_libc::strtol ( args[1] , &end, 16 );
     prog_data[amp_on] = val ;
     msg_console("%s\n" , hex ) ;
}
...
static void amp_slave_command_handler ( TReadLine* rl , TReadLine::const_symbol_type_ptr_t* args , const size_t count )
{
	 if ( count == 1 )
	 {
		 i2hex( prog_data[amp_slave],hex);
		 msg_console("%s\n" , hex ) ;
		 return ;
	 }
	 char* end ;
     uint32_t val = kgp_sdk_libc::strtol ( args[1] , &end, 16 );
     prog_data[amp_slave] = val ;
...
{
#ifdef __LA3_MOD__
	i2hex(6,hex); //PangaeaCP16_PA LA3
#else
	i2hex(3,hex); //PangaeaCP16_PA
#endif
	msg_console("%s" , hex ) ;
	msg_console("\n") ;
	msg_console("END\n") ;
...
void ConsoleSetCmdHandlers(TReadLine* rl)
{
  ...
  rl->AddCommandHandler("gb", get_bp_command_handler);        // чтение BP
...
//  rl->AddCommandHandler("dcc", delete_current_cab_command_handler);
  ...
  rl->AddCommandHandler("pc", direct_program_command_handler); // установка BP
  rl->AddCommandHandler("sp", save_pres_command_handler);
